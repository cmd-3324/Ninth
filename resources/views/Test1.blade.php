...
@php
    $products = implode(', ', ['Car', 'Bike', 'Bus']);
    $products2 = ['Sib','Havij', 'Torshi'];
@endphp
...
@php
    $alert_message = "Get out of the way";
@endphp
{{-- @includeWhen(isset($products), 'Vest', ['product' =>$products]) --}}
@include('Vest', ['alert_message' => $alert_message, 'font_family' => 'sans-serif','product' =>$products])
{{-- @foreach($products2 as $product)
    @include('Vest',['product' => $product])
@endforeach --}}
---each(direcive one , i mean) does not work , but others do!
{{-- @each('Vest', $products2, 'product', 'view.empty')
@each('Vest', $jobs, 'job', 'view.empty') --}}
</body>
</html>
